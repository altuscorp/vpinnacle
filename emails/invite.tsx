...
export function InviteEmail({ inviteeName, inviterName, link }: Props) {
  return (
    <EmailLayout preview={`${inviterName} invited you to VPinnacle Dashboard`}>
      <Heading style={{ fontSize: 20, color: "#0F172A", margin: "0 0 16px" }}>
        Hi {inviteeName},
      </Heading>
      <Text style={{ fontSize: 14, color: "#334155", lineHeight: 1.6, margin: "0 0 16px" }}>
        <strong>{inviterName}</strong> has invited you to the VPinnacle Dashboard —
        the work-management tool VPinnacle Loans is using to track tasks across the team.
      </Text>
