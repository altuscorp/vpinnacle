# Admin perspective — running VPinnacle Dashboard

This walkthrough is for *you*, Manan — the person who owns the system. As an admin you get the full keyboard: inviting people, watching the team's day unfold on the dashboard, and stepping into any task to approve, reassign, cancel or transfer it. Below is every screen you'll see on a demo, in the order you meet them.

---

## A. First login (after invite)

Your account is set up the same way everyone else's is — Hetesh invites you, your inbox gets the email, you set a password. The only difference is the **Admin** flag on your `employees` row, which unlocks an extra menu and side panel.

**The `/login` screen.** Visit the root URL with no session; the `requireUser()` guard in `app/(app)/layout.tsx` bounces you to `/login?next=/`. A centred white card on a soft off-white canvas: a red **ALTUS** pill next to *VPinnacle Dashboard* in serif, the heading **"Sign in"**, the line *"Use the email your admin set up for you,"* two fields — **Email** and **Password** — a red gradient **"Sign in"** button, and a "Forgot password?" link. On success the form calls Firebase Auth, POSTs the ID token to `/api/auth/session` to swap it for a server-side session cookie, and redirects you to `/`.
...
**The Dashboard.** The dark-navy header has two soft radial gradients (red bottom-left, purple top-right) and a rainbow gradient strip across the bottom edge. Left: a red caps line *"Altus Corp · Operations Dashboard"*, the big serif italic *"VPinnacle Loans"* headline (*Loans* in a red gradient), and a shimmering "By Altus Corp" pill. Centre: three nav pills — **Dashboard**, **Tasks** (live count), **Archived** (live count). There's no "Admin" pill; admin lives in the user menu. Right: the red **"+ New task"** button, your avatar, a green/amber **Live** dot for the Supabase Realtime channel, and *"Updated X seconds ago"*.
...
The Admin section has its own layout: a dark sidebar with the VPinnacle/Altus brand block, an identity chip with your avatar, and six nav links — **Overview**, **Activity**, **Notifications**, **Employees**, **Departments**, **Settings** — followed by **"← Back to app"** and **Sign out**.
