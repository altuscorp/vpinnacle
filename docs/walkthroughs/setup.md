# Setup: VPinnacle Dashboard end-to-end

This is the walkthrough for getting the dashboard running on a fresh machine — from zero to "I can sign in as an admin and see real tasks." Follow it top-to-bottom. Every command below is copy-pasteable from the repo root (`D:\vpinnacle`).
...
- **Accounts you will need:**
  ...
  - **Vercel** + **GoDaddy** are *optional* — only if you want to deploy beyond localhost and point `dashboard.altuscorp.in` at it.
...
3. **Project Settings (gear icon) → General → Your apps → Add app → Web (`</>`)**. Nickname it anything. Firebase shows a `firebaseConfig` block — copy these:
   ...
   - `authDomain` → **`NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN`** (e.g. `vpinnacle-dev.firebaseapp.com`)
...
4. Set **`RESEND_FROM_EMAIL`** to either a verified address (e.g. `"VPinnacle Dashboard <dashboard@altuscorp.in>"`) or fall back to `"VPinnacle Dashboard <onboarding@resend.dev>"` (Resend's shared sandbox) for first-light testing.
...
pnpm bootstrap-admin --email manan@altuscorp.in --name "Manan Vasa"
```
...
You'll receive an email like "Reset your VPinnacle password." Click through to `/set-password`, set a password, land on `/welcome`.
