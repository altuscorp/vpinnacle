#!/usr/bin/env tsx
/**
 * One-time importer for the legacy VPinnacle Google Sheet.
...
async function sendInviteEmailLite(args: {
  ...
  const from = process.env.RESEND_FROM_EMAIL || "VPinnacle Dashboard <onboarding@resend.dev>";
  try {
    const { error } = await new Resend(key).emails.send({
      from,
      to: args.email,
      subject: "You've been invited to VPinnacle Dashboard",
      html: `<p>Welcome to VPinnacle Dashboard.  Set your password here:</p><p><a href="${args.resetLink}">${args.resetLink}</a></p>`,
    });
