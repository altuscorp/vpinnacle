...
describe("csvResponse", () => {
  it("returns 200 + text/csv + attachment disposition + UTF-8 BOM", async () => {
    const res = csvResponse({
      filename: "vpinnacle-tasks-2026-05-17.csv",
      headers: ["a", "b"],
      rows: [["1", "2"], ["3", "4"]],
    });
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toMatch(/text\/csv/);
    expect(res.headers.get("content-disposition")).toMatch(/vpinnacle-tasks-2026-05-17\.csv/);
...
describe("exportFilename", () => {
  it("formats as vpinnacle-<resource>-YYYY-MM-DD.csv", () => {
    expect(exportFilename("tasks", new Date("2026-05-17T10:30:00Z"))).toBe(
      "vpinnacle-tasks-2026-05-17.csv",
    );
  });

  it("uses today by default", () => {
    const today = new Date().toISOString().slice(0, 10);
    expect(exportFilename("employees")).toBe(`vpinnacle-employees-${today}.csv`);
  });
});
