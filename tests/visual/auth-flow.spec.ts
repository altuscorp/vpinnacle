...
const PROJECT_ID =
  process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || "vpinnacle-dev";
...
test.describe("M2.0 auth flow", () => {
  ...
  test("admin invites a user; user signs up via invite link", async ({
    ...
    const adminEmail = "manan@altuscorp.in"; // adjust to your seed
    const newEmail = `e2e-${Date.now()}@vpinnacle.com`;
